...
# extract SteamCMD
$ARGUMENTS="Expand-Archive -Force $VARS.RF2ROOT\SteamCMD\steamcmd.zip -DestinationPath ""$VARS.RF2ROOT\SteamCMD"""
start-process -FilePath powershell -ArgumentList $ARGUMENTS -NoNewWindow -Wait

# download and install rFactor 2 dedicated server
$ARGUMENTS="+force_install_dir $VARS.RF2ROOT +login anonymous +app_update 400300 +quit"
start-process -FilePath "$VARS.RF2ROOT\SteamCMD\steamcmd.exe" -ArgumentList $ARGUMENTS -NoNewWindow -Wait

# install vcredist ... will prompt for Admin PW
start-process $VARS.RF2ROOT\support\runtimes\vcredist_2012_x64.exe " /install /quiet /log $HOME\Documents\vcredist2012.log" -Verb runAs -wait 
start-process $VARS.RF2ROOT\support\runtimes\vcredist_2013_x64.exe " /install /quiet /log $HOME\Documents\vcredist2013.log" -Verb runAs -wait


# generating the rfactor 2 dedicated server link ... it is not needed if DS is started from $VARS.RF2ROOT with leading bin64\ ... but
$LNKFILE = "$VARS.RF2ROOT\bin64\rFactor2 Dedicated.lnk"
$WshShell = New-Object -comObject WScript.Shell
$Shortcut = $WshShell.CreateShortcut("$LNKFILE")
$Shortcut.TargetPath = "$VARS.RF2ROOT\bin64\rfactor2 dedicated.exe"
$Shortcut.Arguments = "+path="".."""
$Shortcut.WorkingDirectory = "$VARS.RF2ROOT\bin64"
$Shortcut.Save()

# downloading track and car initial mod
start-process $VARS.RF2ROOT\SteamCMD\steamcmd.exe "+force_install_dir $VARS.RF2ROOT +login anonymous +workshop_download_item 365960 2362626369 +quit" -nonewwindow -wait
start-process $VARS.RF2ROOT\SteamCMD\steamcmd.exe "+force_install_dir $VARS.RF2ROOT +login anonymous +workshop_download_item 365960 917386837 +quit" -nonewwindow -wait

write-host "Starting Mod Manager in order to set modmgr folders (closing in a couple of seconds)."
start-process $VARS.RF2ROOT\bin64\modmgr.exe "-q -c""$VARS.RF2ROOT"" "
...
# installing content
$FOLDERS=((gci $VARS.RF2ROOT\steamapps\workshop\content\365960\*).Name)
forEach ($FOLDER in $FOLDERS) {
 $RFCMPS=(gci $VARS.RF2ROOT\steamapps\workshop\content\365960\$FOLDER\*.rfcmp|select -Expand Name| sort)
 foreach ($RFCMP in $RFCMPS) {
  $ARGUMENTS=" -i""$RFCMP"" -p""$VARS.RF2ROOT\steamapps\workshop\content\365960\$FOLDER"" -d""$VARS.RF2ROOT"" -c""$VARS.RF2ROOT"" -o""$VARS.RF2ROOT"" "
  start-process -FilePath "$VARS.RF2ROOT\bin64\ModMgr.exe" -ArgumentList $ARGUMENTS -nonewwindow -wait
 }
}
...
CurLocation=0
NumLocations=1
Location=$VARS.RF2ROOT\Packages\dummy.rfmod
CurRFM=0
NumRFM=1
RFM=$VARS.RF2ROOT\Packages\dummy.mas
NumTrackFiles=1
...
NumOtherFiles=0
rFmFile=$VARS.RF2ROOT\Packages\default.rfm
IconFile=$VARS.RF2ROOT\Packages\icon.dds
SmallIconFile=$VARS.RF2ROOT\Packages\smicon.dds

CurPackage=0
"@

Set-Content -Path $VARS.RF2ROOT\Packages\dummy.dat -Value $PKGINFO -Encoding ASCII
timeout /t 5|out-null

ForEach($FILE in 'default.rfm', 'icon.dds', 'smicon.dds')
{
cp $CURRENTLOCATION\rfmodfiles\$FILE $VARS.RF2ROOT\Packages
}

# create MAS file
$ARGUMENTS="-q -c""$VARS.RF2ROOT"" -o""$VARS.RF2ROOT\Packages"" -m""dummy.mas"" ""$VARS.RF2ROOT\Packages\default.rfm"" ""$VARS.RF2ROOT\Packages\icon.dds"" ""$VARS.RF2ROOT\Packages\smicon.dds"" "
start-process -FilePath "$VARS.RF2ROOT\bin64\ModMgr.exe" -ArgumentList $ARGUMENTS -NoNewWindow -Wait

timeout /t  5|out-null

# create RFMOD package
$ARGUMENTS="-q -c""$VARS.RF2ROOT"" -o""$VARS.RF2ROOT\Packages"" -b""$VARS.RF2ROOT\Packages\dummy.dat"" 0"
start-process -FilePath "$VARS.RF2ROOT\bin64\ModMgr.exe" -ArgumentList $ARGUMENTS -NoNewWindow -Wait
timeout /t 5|out-null

# install RFMOD, TODO: respect exit codes
$ARGUMENTS="-q -p""$VARS.RF2ROOT\Packages"" -i""dummy.rfmod"" -c""$VARS.RF2ROOT"" "
start-process -FilePath "$VARS.RF2ROOT\bin64\ModMgr.exe" -ArgumentList $ARGUMENTS -NoNewWindow -Wait
timeout /t 5|out-null
...
if (-not (Test-Path "$VARS.RF2ROOT\Userdata\player")) {
 mkdir $VARS.RF2ROOT\Userdata\player
}

$ARGUMENTS=" +profile=player +rfm=dummy_10.rfm +oneclick"
start-process -FilePath "$VARS.RF2ROOT\bin64\rFactor2 Dedicated.exe" -ArgumentList $ARGUMENTS -NoNewWindow
